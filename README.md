...
UI에서 실행하는 agent에는 `cowork-agent/CLAUDE.md`의 응답 지침(2026-09-10)을
매 요청에 전달합니다. 최종 답변은 1500토큰 이내로 결론 → 근거 → 다음 조치 순서로 쓰고,
중간 탐색·시행착오, 과장 표현과 챗봇 상투어를 제외합니다. 긴 표·코드는 별도 산출물로
저장하고 답변에는 경로를 링크합니다. 산출물 자체의 분량은 제한하지 않습니다.

지침은 `backend/app/response_policy.py`에서 관리하며 새 대화, 기존 대화의 후속 요청,
압축 후 재개에 적용합니다. 내부 문맥 압축에는 적용하지 않습니다.
모델에 전달하는 작성 지침이므로 토큰 수를 계산해서 답변을 강제로 자르지는 않습니다.
...
상단 `CLAUDE` 사용량 옆 갱신 아이콘을 누르면 현재 계정으로 짧은 Claude 확인 호출을 합니다.
확인 호출에는 소량의 사용량이 발생하며, 화면 진입·계정 전환·시간 경과에 따른 자동 호출은
없습니다. 확인 중에는 버튼을 비활성화하고, 여러 탭의 동시 요청은 같은 확인 호출을 공유합니다.
실패하면 마지막 수신 값을 유지하고 오류를 표시합니다. 갱신 아이콘을 눌러 다시 시도할 수 있습니다.

### APM (Scouter) — 운영 화면의 실시간 수치

운영 화면은 WEB/WAS 토폴로지 패널 없이 **APM · Scouter** 패널만 표시합니다.
헤더의 점검 아이콘은 기존 WEB/WAS 로그 점검 에이전트를 새 세션으로 실행하고, 읽기 아이콘은
Scouter 지표만 갱신합니다. 두 아이콘은 마우스 오버·키보드 포커스 시 툴팁으로 설명합니다.
TPS · 응답시간 · Heap · CPU 는 agent 가 아니라 **백엔드가 직접** 읽으므로 자동 읽기에도
토큰을 사용하지 않습니다. 로그 점검은 별도의 수동 실행이며 토큰을 사용합니다.
...
프롬프트 창에서 작업을 요청하면 main agent가 받습니다. subagent 선택 메뉴는 없으며,
main agent가 요청과 프로젝트 문맥을 보고 적절한 subagent에 업무를 할당하고 결과를 종합합니다.
해당 업무의 plan agent가 있으면 기존 plan → impl → eval 절차로 위임합니다.
하네스에서 agent를 선택하거나 단계를 바꾸는 동작은 화면 탐색에만 쓰이며 요청 대상을 바꾸지 않습니다.
기존 세션에 이어 보내는 요청과 답변, 세션 다시 열기도 main agent로 전달됩니다.
...
- 모델: Default(세션 기본값) · Opus 5 · Fable 5.1 · Sonnet 5 · Haiku 4.5. 이름으로 거를 수 있습니다.
