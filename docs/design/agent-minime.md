...
- **`/compact` · 콘솔 「압축」**: 지금 세션에 인계 문서를 한 턴 받고(도구 금지·답으로만·3000자),
  그 답을 `handoff` 에 담아 새 세션으로 옮긴다. 입력판에 "인계 첨부" 쪽지가 뜨고, 다음 지시문
  **뒤에** 붙어 나간다(제목은 지시문 첫 줄이라 사람 말이 앞). 떼어 낼 수 있다. 앞 세션은 그대로
  남는다. CLI 내장 `/compact` 를 안 쓰는 이유: print 모드에서 처리되는지 확인된 바 없고, 처리돼도
  무엇이 남고 빠졌는지 화면이 볼 수 없다.
- **`/clear`**: 콘솔의 「새 세션」과 같다. 슬래시 명령은 CLI 로 보내지 않고 화면에서 가로챈다.
