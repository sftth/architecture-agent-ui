...
- **`/compact` · 입력판 위 압축 아이콘**: 새 세션을 열고, 첫 지시문 **뒤에** "앞 세션을 압축했다.
  `report/{project}` 의 최신 보고서를 먼저 읽고 이어서 진행한다" 한 줄을 붙인다. 인계 문서를 따로
  만들지 않는다 — 맥락은 agent 가 남기는 보고서가 잇는다(사용자 결정). 앞 세션과 지시문 이력은
  세션 목록에 그대로 남는다.
  - CLI 내장 `/compact` 는 쓸 수 없다. `claude -p "/compact" --resume …` 을 실제로 보내 보니(2026-09-04,
    haiku 임시 세션) 처리되지 않고 모델이 그 글자를 경로로 읽어 "이 경로로 무엇을 할까요"라고
    되물었다. print 모드는 내장 슬래시 명령을 받지 않는다.
- **`/clear` · 입력판 위 지우개 아이콘**: 빈 새 세션. 아무것도 붙이지 않는다.
- 슬래시 명령은 CLI 로 보내지 않고 화면에서 가로챈다. 아이콘은 입력칸 바로 위 오른쪽 끝에 있다 —
  무거워진 대화를 끊는 결정은 다음 말을 적기 직전에 내려진다.
