...
## 8. 세션 압축과 clear (2026-09-04 추가)

턴마다 `claude -p --resume` 으로 새 프로세스가 뜨므로 대화 전체가 매번 다시 들어간다. 길어질수록
토큰이 새고, 한도 근처에서는 CLI 가 스스로 압축하며 세부를 잃는다. 언제 정리할지는 사람이 정한다.

- **문맥 게이지** (`context.ts`, 콘솔 머리): 마지막 `result` 이벤트의 `usage.iterations` 마지막 항목
  (입력 + 캐시 읽기 + 캐시 쓰기) = 마지막 API 호출의 문맥. 한도는 `modelUsage[*].contextWindow`.
  60% 를 넘으면 amber. 실제 로그로 확인: 175k / 1.0M.
- **`/compact` · 입력판 위 압축 아이콘** (2026-09-04 세 번째 개정 — CLI 와 같은 동작): 백엔드
  `compact_run` 이 CLI 의 압축과 같은 일을 한다. 지금 대화에 "이어갈 수 있게 요약하라"(도구 금지)를
  한 턴 보내고, 답을 받아 두고 CLI 대화 식별자를 새로 딴다. 다음 지시문은 그 요약을 첫 메시지로 삼은
  **새 CLI 대화**로 열린다(`COMPACT_HEADER` + 요약 + 지시문, `--session-id`). 화면에서는 같은 세션이고
  지시문 이력도 그대로다 — 문맥만 줄어든다. 요약 턴은 지시문 수에 넣지 않고, 그 답(요약)은 콘솔에
  세우지 않는다(CLI 처럼). 시작·끝은 `system` 이벤트 `compact_start` / `compact_end` / `compact_failed`
  로 남고 콘솔에 "압축" 한 줄로 선다. 보고서를 읽으라는 말은 붙이지 않는다 — 그건 사용자가 필요할 때
  직접 시킨다.
  - CLI 내장 `/compact` 는 쓸 수 없다. `claude -p "/compact" --resume …` 을 실제로 보내 보니(haiku 임시
    세션) 처리되지 않고 모델이 그 글자를 경로로 읽어 되물었다. print 모드는 내장 슬래시 명령을 받지 않는다.
- **문맥 게이지는 입력판 위 왼쪽**(콘솔 머리에서 옮김). CLI 처럼 칸 20개를 채워 보이고 "17% · 175k / 1.0M".
  압축 중이면 칸이 차례로 숨을 쉬며 "압축 중…", 압축 직후에는 "압축됨 · 175k → 0" 을 다음 턴이 새 크기를
  말해 줄 때까지 보인다.
- **`/clear` · 입력판 위 지우개 아이콘**: 빈 새 세션. 아무것도 붙이지 않는다.
- 슬래시 명령은 CLI 로 보내지 않고 화면에서 가로챈다. 아이콘 툴팁은 명령 이름만.
- **숨긴 stderr**: `Sandbox disabled`(조직 설정이 sandbox 를 켰지만 Windows 에 그 기능이 없어 CLI 가
  프로세스마다 알림)와 stdin 3초 대기 안내는 콘솔에 세우지 않는다. 사람이 할 수 있는 일이 없고
  턴마다 되풀이되어 진짜 경고를 묻었다.
